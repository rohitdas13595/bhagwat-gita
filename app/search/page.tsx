...
export default async function SearchPage({
  ...
  return (
    <div className="flex flex-col gap-10 pb-24 animate-in fade-in duration-700 max-w-5xl mx-auto w-full">
      {/* Search Header Form */}
      <div className="relative overflow-hidden rounded-2xl border border-[#222] bg-gradient-to-br from-[#1b1229] to-[#0A0A0A] p-10 md:p-14">
        <h1 className="text-4xl md:text-5xl font-bold font-serif mb-6 text-white text-center">
          Search the Gita
        </h1>
        <form
        ...
        >
          ...
          <input
            ...
            name="q"
            defaultValue={q || ""}
            className="w-full bg-[#121212] border border-[#333] text-white text-lg rounded-full focus:ring-[#A87FFB] focus:border-[#A87FFB] block pl-12 pr-4 py-4 shadow-xl transition-all"
      ...
      {!query ? (
        ...
        <div className="flex flex-col gap-12">
          {/* Results Summary */}
          <div className="text-xl text-zinc-300 font-serif border-b border-[#222] pb-4">
            Found{" "}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {matchedChapters.map((chapter) => (
                  <Link
                    ...
                    className="p-6 rounded-xl border border-[#222] bg-[#121212] hover:bg-zinc-900 transition-all hover:-translate-y-1 group"
                  >
                    <div className="text-[#A87FFB] font-bold text-sm mb-2 uppercase tracking-widest">
                      Chapter {chapter.chapter_number}
                    </div>
                    <h3 className="text-xl font-serif text-white mb-2 group-hover:text-[#A87FFB] transition-colors">
              ...
              <div className="flex flex-col gap-4">
                {matchedVerses.map((verse) => (
                  <Link
                    ...
                    className="flex flex-col md:flex-row items-start gap-4 p-6 rounded-xl border border-[#222] bg-[#121212] hover:bg-zinc-900 transition-all hover:-translate-y-1 group"
                  >
                    <div className="text-black bg-white rounded flex items-center justify-center font-bold px-3 py-1 text-sm whitespace-nowrap">
                      {verse.chapter_number}.{verse.verse_number}
                    </div>
                    <div className="flex-1">
                      <p className="font-serif text-lg text-white mb-2 leading-relaxed opacity-90 group-hover:opacity-100 transition-opacity">
          ...
          {matchedChapters.length === 0 && matchedVerses.length === 0 && (
            <div className="text-center text-zinc-500 py-10 bg-[#121212] border border-[#222] rounded-xl">
